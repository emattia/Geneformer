...
def initialize_ray_with_check(ip_address = None):
    """
...
DISEASE_TRAIN_DATA_PATH = os.path.abspath("../data/human_dcm_hcm_nf.dataset")
PRETRAINED_MODEL_PATH = os.path.abspath('../geneformer-12L-30M')
OUTPUT_PATH = os.path.abspath('../tuned-disease-classification-finetuning-output')

assert os.path.exists(DISEASE_TRAIN_DATA_PATH), f'DISEASE_TRAIN_DATA_PATH {DISEASE_TRAIN_DATA_PATH} does not exist'
...
train_dataset=load_from_disk(DISEASE_TRAIN_DATA_PATH)
...
output_dir = f"{OUTPUT_PATH}/{datestamp}_geneformer_DiseaseClassifier/"
...
epochs = 1
# logging steps
logging_steps = 1 # round(len(classifier_trainset)/geneformer_batch_size/10)
...
trainer.hyperparameter_search(
    direction="maximize",
    backend="ray",
    resources_per_trial={"cpu":3, "gpu":.5},
